...
public class StreamCipher {
    ...
    // Instance variables.
    private PRGen generator;
    private final byte[] nonce = new byte[NONCE_SIZE_BYTES];
    private byte[] key;
    ...
    public StreamCipher(byte[] key, byte[] nonceArr, int nonceOffset) {
        ...
        this.key = key;
        System.arraycopy(nonceArr, nonceOffset, nonce, 0, NONCE_SIZE_BYTES - 1);
        PRF prf = new PRF(key);
    ...
    public byte cryptByte(byte in) {
        return (byte) (generator.next(8) ^ in);
    }
    ...
    public void cryptBytes(byte[]  inBuf, int  inOffset, 
        ...
        int outCount = outOffset;
        for (int inCount = inOffset; inCount < inOffset + numBytes; inOffset++) {
            outBuf[outCount] = cryptByte(inBuf[inCount]);
            outCount++;
        }
    }
}
